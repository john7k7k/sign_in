...
module.exports = function(sql_data = {
    ...
    connection.buildVideoTable = async function (){
        return new Promise(async (reslove, reject)=>{
    ...
    connection.getVideoTable = async function(section = '001'){
        return new Promise(async (reslove, reject)=>{
            let query = '';
            if(section === '001') query = `SELECT * FROM Video`;
            else query = `SELECT * FROM Video WHERE section = '${section}'` ;
            connection.buildVideoTable();
            this.query(query, (err, result) => {
                if (err) reject(err);
                else {
                    reslove(result)
    ...
    connection.showVideoTable = async function(section = '001'){
        return new Promise(async (reslove, reject)=>{
            console.log('Video table:');
            reslove(console.table(await connection.getVideoTable(section)));
        })
    }
    connection.updateVideo = async function (video_data){
        return new Promise((reslove, reject)=>{
            const query = 'INSERT INTO Video SET ? ON DUPLICATE KEY UPDATE ?';
            connection.query(query, [video_data, video_data], (err, results) => {
              if (err) {
    ...
    connection.deleteVideoTable = async function (){
        return new Promise((reslove, reject)=>{
            this.query(`DROP TABLE IF EXISTS Video`, (err, result) => {
                if (err) reject(err);
                else {
                    reslove(result)
                }
